...
# Standardized interface script for Fickling scan tool
# Usage: ./run.sh --model /path/to/model --output /path/to/output

# Default values
MODEL_PATH=""
OUTPUT_PATH="fickling_results.txt"
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --help)
      ...
      echo "  --output PATH   Path to save the scan results (default: fickling_results.txt)"
...
# Run the Fickling scan
echo "Running Fickling scan on $MODEL_PATH..."
python fickling_scan.py "$MODEL_PATH" "$OUTPUT_PATH"
